...
totalbatch = int(trainX1.shape[0] / batch_size)
training_data_count = len(trainX1)
...
def self_attention(x, weight_adj):
    f = np.dot(x,weight_adj['w1'])
    g = np.dot(x,weight_adj['w2'])

    alpha = g * f

    alpha = softmax(alpha)  # attention map
    return alpha
...
weight_adj={
    'w1':np.random.randn(num_nodes,num_nodes)*0.1,
    'w2':np.random.randn(num_nodes,num_nodes)*0.1}

# ad = np.corrcoef(adj, rowvar=False)

att = self_attention(adj, weight_adj)

np.set_printoptions(threshold=np.inf)
# print(adj)

data2 = np.mat(data, dtype=np.float32)
# print(data2[0].shape)
...
# rmse = math.sqrt(mean_squared_error(data1, data2))
# print(rmse)
